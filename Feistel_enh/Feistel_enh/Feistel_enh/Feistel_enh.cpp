// Feistel_enh.cpp: определяет точку входа для консольного приложения.
//
...
unsigned long long encipher(unsigned long long word, int rounds,unsigned long long key, unsigned short iv);
unsigned long long decipher(unsigned long long word, int rounds,unsigned long long key, unsigned short iv);


int _tmain(int argc, _TCHAR* argv[])
{	
    unsigned long long word = 12345;
    unsigned long long key = 65432789645678;
	unsigned short iv = rand() % RAND_MAX + 10000;
	//cout << iv << endl;
    int rounds = 3;
	cout << word << endl;
	word = encipher(word, rounds, key, iv);
    cout << word << endl;
	word = decipher(word, rounds, key, iv);
	cout << word << endl;
	cin.ignore();
	return 0;
}
...
unsigned long long decipher(unsigned long long word, int rounds,unsigned long long key, unsigned short iv) {
    ...
    for (int i = rounds - 1; i > 0; i--) { 
		...
		a = (unsigned short*)&result;
		b = (unsigned short*)&tmp;		
		a[3] = b[3] ^ b[1] ^ round_key;
		a[2] = b[2] ^ b[3];
		a[1] = reversed_f(b[1] ^ b[0]);
		a[0] = b[0] ^ iv;
		tmp = result;
        if (i != 0) {
			result = 0;
